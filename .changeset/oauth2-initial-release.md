...
- `createOAuth` — the relying-party flow (PKCE S256, `state` / `nonce` generated and verified, exact `redirect_uri` + RFC 9207 `iss` matching) with 12 provider presets (`./providers/*`: google, github, microsoft, azure, discord, facebook, linkedin, spotify, twitch, apple, twitter, okta).
