...
## OAuth 2 token endpoints (`@exortek/opaque`)

| §                | Requirement                                                        | Status | How                                                                                              |
|------------------|--------------------------------------------------------------------|:------:|--------------------------------------------------------------------------------------------------|
| RFC 7662 §2.2    | Token introspection endpoint — JSON `{ active, ... }` response     |   ✅    | `introspectionHandler` — always 200, `active: false` for anything invalid (no oracle)            |
| RFC 7662 §2.1    | Introspection endpoint MUST authenticate the caller                |   🟡   | Docs require it; the handler ships un-gated so the app can pick mTLS / basic / HMAC              |
| RFC 7009 §2.2    | Token revocation endpoint responds regardless of token validity    |   ✅    | `revocationHandler` — always 204, empty body, so the endpoint cannot probe validity              |
