...
<Callout type="info">
  **NIST SP 800-63B guidance** (2020 revision) recommends **against**
  ...
  `Password1!`. Prefer `minLength` + `requireMinScore` over
  `requireClasses` unless a compliance regime forces your hand. Neither
  `strength` nor `assertPolicy` checks against a common-password list —
  pair with [`createHibpClient`](/password/hibp) (HIBP, k-anonymity) if
  you need that.
</Callout>
