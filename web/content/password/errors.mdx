...
`@exortek/password` throws exactly these 6 codes. Verifying a wrong password is **not** one of them — `verify` /
`constantTimeVerify` return `false` rather than throw (see [When errors are thrown](#when-errors-are-thrown)).
