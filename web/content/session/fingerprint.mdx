...
on your framework's `trustProxy` setting — see [middleware](/session/middleware)
...
- **Hono** — reaches into `c.env.incoming?.socket?.remoteAddress`
  (Node adapter). Edge runtimes without an `incoming` object fall
  back to UA-only.
- **Elysia** — calls `context.server?.requestIP(context.request)`.
