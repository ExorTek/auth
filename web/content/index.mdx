...
A framework-agnostic, zero-dependency authentication toolkit for
Node.js — built as 22 small packages under one scope. Pick the one
you need, or install the umbrella.
...
Numbers reflect dependency order — a lower number never imports from
a higher one, so you can adopt one package at a time.
...
- **[`@exortek/crypto` reference →](/crypto)** — every export, every
  option, every edge case for the package shipping today.
- **[Source](https://github.com/ExorTek/auth)** — GitHub repo.
- **[Contributing](https://github.com/ExorTek/auth/blob/master/CONTRIBUTING.md)**
  — branch prefixes, commit conventions, changeset flow.
