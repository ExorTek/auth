...
<Callout type="warning">
  **Minimum key material:**
  ...
  - **RSA (RFC 7518 §3.3 / §3.5)** — RS/PS keys must have a modulus of at
    least **2048 bits**. Enforced whether the key arrives as a `KeyObject`
    or a JWK.
...
**Detached + `b64: false`** — the canonical form for `x-jws-signature`
(Open Banking) and JAdES. Set both options together; the emitted token
still has an empty payload segment, and the signing input is
`encHeader.rawBytes` (not base64url). `.` bytes in the payload are fine
here because the compact form has no ambiguity when the payload segment
is empty.
...
<Callout type="warning">
  ...
  are guarded; a `.` raises `INVALID_PAYLOAD`. (The detached tab shows the
  b64:false + detached combination where the guard does not apply.)
</Callout>
