...
existing codes are never renamed or repurposed. `@exortek/otp` throws
exactly these 3 — replay detection and rate-limiting are **your**
composition (this package doesn't ship a store or a limiter), not a
built-in `OtpError` code.

| Code | Meaning | Status |
| --- | --- | --- |
| `INVALID_ARGUMENT` | Type / range / shape validation failed on an option (bad `digits`, `period`, `window`, missing `label`, etc.). | `400` |
| `UNSUPPORTED_ALGORITHM` | Algorithm not in `SHA1 / SHA224 / SHA256 / SHA384 / SHA512`, or a `provisioningUri` call with `SHA224` / `SHA384`. | `400` |
...
| `provisioningUri` | `INVALID_ARGUMENT` (missing `label` / `secret`, bad `type`), `UNSUPPORTED_ALGORITHM` (`SHA224` / `SHA384`) |
...
function toHttpStatus(err) {
  ...
  switch (err.code) {
    ...
    case ErrorCode.INVALID_SECRET:        return 401
    default:                              return 500
  }
}
```
