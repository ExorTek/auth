...
// Two separate stores — the limiter's own counter store, and the
// replay-guard store `verifyTotp` uses. `rateLimit.sliding()` returns
// only `{ check }`, no `.store` property, so keep your own reference.
const replayStore = rateLimit.stores.memory()   // Redis in multi-worker deploys
const throttle = rateLimit.sliding({
  requests: 5, window: '10m',
  store: rateLimit.stores.memory(),
})

// Throttling is your application's concern, not `@exortek/otp`'s — there's
// no `ErrorCode.THROTTLED` on `OtpError`. Handle the deny however fits
// your stack (a plain Error, a 429 response, etc.).
async function verify(userId, code, secret) {
  const rl = await throttle.check({ key: `otp:${userId}` })
  if (!rl.allowed) {
    throw new Error(`Too many attempts. Try again in ${rl.retryAfter}s.`)
  }
  return verifyTotp(code, secret, {
    window: 1,
    replay: { store: replayStore, key: `user:${userId}` },
  })
}
```
