 ...
 *
 * Every entry carries what `sign` / `verify` need to route to the
 * right `node:crypto` primitive without the surface files having to
 * branch by alg themselves.
 *
 ...
 * throws — consumers surface this as their own `UNSUPPORTED_ALGORITHM`
 * error code with a dedicated `ALGORITHM_NONE_FORBIDDEN` fast-path in
 * the sign / verify wrappers.
 */
...
/** Every JOSE algorithm identifier this registry supports. */
export const SUPPORTED = Object.freeze(Object.keys(TABLE));

/**
 * Look up an algorithm's descriptor. Throws for anything not in the
 * table, including the placeholder `'none'` alg. Consumers wrap the
 * throw as their own typed error at the surface boundary.
 *
 * @param {string} alg
...
export function lookup(alg) {
  ...
  if (!entry) {
    throw new Error(`algorithms.lookup: unsupported alg ${JSON.stringify(alg)}. Supported: ${SUPPORTED.join(', ')}`);
  }
  return entry;
}
