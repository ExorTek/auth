 ...
 * authorization server that serves the matching JWKS, discovery document,
 * token, and userinfo endpoints. No network, no live provider. Adapted from
 * the `@exortek/oauth2` test helper.
 */
...
/**
 * Generate an EC P-256 signing key and return a signer plus its public JWK
 * (for the stub JWKS).
 *
 * @param {{ kid?: string, alg?: string }} [opts]
 */
export async function makeSigner({ kid = 'test-key-1', alg = 'ES256' } = {}) {
  ...
  return { publicJwk, privateJwk, mint, kid, alg };
}

/**
 * Start a stub AS. Pass the public JWK(s) to publish and per-endpoint
 * handlers. Returns `{ base, issuer, jwksUri, close }`.
 *
 * @param {{
 *   publicJwks: object[],
