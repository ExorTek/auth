...
export function derToRaw(der, curve) {
  ...
  if (!size) {
    throw new JwtError(ErrorCode.INVALID_SIGNATURE, `ecdsa.derToRaw: unknown curve ${JSON.stringify(curve)}`);
  }
  if (!Buffer.isBuffer(der) || der.length < 8) {
    throw new JwtError(ErrorCode.INVALID_SIGNATURE, 'ecdsa.derToRaw: DER is truncated');
  }
  let offset = 0;
  if (der[offset++] !== 0x30) {
    throw new JwtError(ErrorCode.INVALID_SIGNATURE, 'ecdsa.derToRaw: expected DER SEQUENCE');
  }
  ...
  if (r.length > size || s.length > size) {
    throw new JwtError(ErrorCode.INVALID_SIGNATURE, `ecdsa.derToRaw: r/s exceed ${size} bytes for ${curve}`);
  }
...
export function rawToDer(raw, curve) {
  const size = EC_COORD_BYTES[curve];
  if (!size) {
    throw new JwtError(ErrorCode.INVALID_SIGNATURE, `ecdsa.rawToDer: unknown curve ${JSON.stringify(curve)}`);
  }
