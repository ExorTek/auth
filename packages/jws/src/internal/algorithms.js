 ...
 *
 * Each entry carries everything `sign` / `verify` need to route to the
...
function _eddsa() {
  return {
    ...
    kty: 'OKP',
    async sign(key, message) {
      // Node signs Ed25519 / Ed448 with `null` digest — the algorithm is
      // determined by the key material itself (RFC 8032).
