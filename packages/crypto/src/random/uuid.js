...
/** Canonical UUID format matcher (8-4-4-4-12, hex, case-insensitive). Version/variant bits are NOT checked — custom namespace UUIDs may carry any version. */
 ...
 * or variant bits — accepts any well-formed UUID.
 *
 * @param {unknown} value
 * @returns {boolean}
 *
 ...
 * 42-bit random.
 *
 * RFC 9562 specifies 74 random bits after the timestamp; this implementation
 * uses **§6.2 Method 1 (monotonic counter)** — explicitly permitted by the
 * spec — replacing 32 of those bits with a sequence counter to guarantee
 * strict per-process ordering. The remaining 42 bits stay random for
 * collision resistance.
 *
 * Lexicographically sortable by creation time — excellent database primary
 * key (B-tree friendly, no page-split storms like fully random v4).
 *
 * NOT natively provided by `node:crypto` as of Node 22 — composed from
 * `Date.now()` + `crypto.randomBytes(10)` with version/variant bits set per spec.
 *
 ...
 * always), even when invoked at the same millisecond. On counter overflow
 * the timestamp is bumped by one ms to preserve ordering. System clock
 * regressions are ignored — we continue issuing UUIDs using the last
 * observed timestamp, never reusing or moving backwards.
 *
 ...
 */
// Module-level monotonic state. Two successive uuid7() calls within the same
// millisecond increment `_seq` instead of re-rolling random — guarantees
// strict lexical ordering: uuid7() < uuid7() always (per process).
// On 32-bit counter overflow we bump `_lastMs` forward by 1 ms to preserve
// monotonicity (RFC 9562 §6.2 "Method 1" — fixed-length counter).
// `_lastMs = -1` sentinel marks "never observed" so the first call always
// takes the new-ms branch.
let _lastMs = -1;
let _seq = 0;
...
export function uuid5(namespace, name) {
  ...
  const digest = crypto.createHash('sha1').update(namespaceBytes).update(nameBytes).digest();

  // Use first 16 bytes of the 20-byte SHA-1 digest; set version/variant per RFC 9562 §5.5.
  digest[6] = (digest[6] & 0x0f) | 0x50; // version 0101 (v5)
  digest[8] = (digest[8] & 0x3f) | 0x80; // variant 10

  return _stringifyUUID(digest.subarray(0, 16));
}
