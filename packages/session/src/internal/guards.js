...
import { makeInvalidArgument, makeParse, makeAssertNonEmptyString, makeAssertObject } from '@exortek/shared/asserts';
import { SessionError, ErrorCode } from '../errors.js';

/**
 * Bind every guard this package calls to SessionError, so an argument-shape
 * failure anywhere carries one class and one code to branch on.
 *
 * @type {import('@exortek/shared/asserts').WrapFn}
 */
const wrap = (message, extra) => new SessionError(ErrorCode.INVALID_ARGUMENT, message, extra);
